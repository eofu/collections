package com.myself.jdkdemo.rbtree;
...
public class RBTree<K extends Comparable<K>, V> {
	...
	// 左旋
	private void leftRotated(RBNode p) {
		if (p != null) {
			RBNode r = p.right;
	...
	//右旋
	private void rightRotated(RBNode p) {
		if (p != null) {
			RBNode r = p.left;
	...
	// 插入
	public RBNode put(int key) {
		RBNode node = new RBNode(key, null);

		Assert.checkNonNull(node);
		// 插入为根节点
		if (root == null) {
			root = node;
			return root;
	...
	public RBNode remove(Object key) {
		RBNode node = getRBNode(key);
		if (node == null) {
			return null;
		}
		RBNode oldRBNode = node;
		deleteRBNode(node);
		return oldRBNode;
	}

	private void deleteRBNode(RBNode node) {
		// 3、有两个子节点
		if (rightOf(node) != null && leftOf(node) != null) {
			RBNode secessor = secessor(node);
		...
		//			  \
		//		       10
		RBNode replacementNode = leftOf(node) == null ? leftOf(node) : rightOf(node);
		// 2、只有一个子节点。替代节点不为空。
	...
	private void fixAfterDeletion(RBNode node) {
		while (node != root && node.color == BLACK) {
			...
			if (node == leftOf(parentOf(node))) {
				// 红黑树上的兄弟节点
				RBNode ribro = rightOf(parentOf(node));
	...
	// 获取节点
	private RBNode getRBNode(Object key) {
	...
	private RBNode setColor(RBNode node, boolean color) {
		Assert.checkNonNull(node);
		node.color = color;
		return node;
	}
	...
	// 后继节点
	private RBNode secessor(RBNode node) {
		if (node.right != null) {
			RBNode p = node.right;
			while (p.left != null) {
				p = p.left;
